...
# 终极 CSS 修复：强制侧边栏深色背景+纯白文字，按钮高亮，情报卡片清晰
st.markdown("""
    <style>
    /* 1. 全局背景 */
    ...
    .news-box { 
        ...
        border-left: 5px solid #00d1ff; margin-bottom: 15px; color: #ffffff;
    }
    .report-card { 
...
def push_whatsapp(phone, apikey, text):
    # 使用 CallMeBot API 进行 WhatsApp 推送
    if phone and apikey:
...
with st.sidebar:
    ...
    # WhatsApp 配置板块
    st.subheader("📱 移动预警绑定")
    wa_num = st.text_input("WhatsApp 号码 (e.g. 86138...)", placeholder="含国家代码")
...
if df is not None:
    ...
    # D. 双模型共识分析 + WhatsApp 同步
    st.divider()
    if st.button("🚀 启动全球策略共识分析 (Cross-Model PK)"):
        if not or_key: st.error("请填入 OpenRouter API Key")
        else:
            with st.spinner("Claude 3.5 与 Gemini 2.0 正在进行逻辑交叉对比..."):
                ...
                if wa_num and wa_key:
                    alert_text = f"LEAN Pro 预警: {target} 触发共识分析。现价: {price}。建议头寸: {pos_size:,.0f}。请登录终端查看 Claude 与 Gemini 的对冲报告。"
